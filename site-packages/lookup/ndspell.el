...
(defcustom ndspell-ispell-program-arguments nil
  "*Extra arguments for ispell process."
  :type '(choice (const :tag "No extra argument" nil)
		 (repeat :tag "Add arguments" string))
  :group 'ndspell)
...
(put 'ndspell :methods '(exact prefix suffix substring wild regexp))
...
(defun ndspell-dictionary-search (dictionary query)
  (when (or (not lookup-search-found) lookup-force-update)
    (let ((string (lookup-query-string query)))
      (when (or (equal (find-charset-string string) '(ascii))
...
(defun ndspell-dynamic-search (reference)
  (let ((self (lookup-entry-dictionary reference))
    ...
    (unless entries
      (setq entries 
	    (list 
	     (lookup-make-entry self (lookup-entry-code reference)
				(lookup-entry-heading reference)))))
    entries))

(put 'ndspell 'content 'ndspell-dictionary-content)
(defun ndspell-dictionary-content (dictionary entry)
  (format "There is no entries hit with %s." (lookup-entry-code  entry)))
  ...
  (unless (and ndspell-process (eq (process-status ndspell-process) 'run))
    ...
    (let ((buffer (lookup-open-process-buffer " *ndspell*")))
      (setq ndspell-process
	    (apply 'start-process
		   "ndspell" buffer ndspell-ispell-program
		   "-a" "-m" "-C" ndspell-ispell-program-arguments))
