;;; ndeb-binary.el --- binary data support for ndeb agent -*- coding: utf-8 -*-
...
(defcustom ndeb-binary-bmp-display-type
  (or (unless (lookup-inline-image-p 'bmp)
	(or (and (lookup-inline-image-p 'ppm) 'ppm)
      ...
      'bmp)
  "Image type to which bmp image is converted for displaying."
...
"A list of lists which consist of key and types to play binary
links from lookup-entry mode."
:type '(repeat
	(list
	 (string :tag "Key")
...
(declare-function ndebs-select-dictionary "ndebs" (dictionaty))
(declare-function ndebs-require "ndebs" (string &optional filter))
...
(defun ndeb-binary-insert-color-image (dictionary type target &optional start end)
  ...
  (when (lookup-inline-image-p type)
    (let ((file (ndeb-binary-bind-temporary-file dictionary type target nil))
	  (lookup-max-image-size ndeb-max-image-size)
	  (lookup-max-image-width ndeb-max-image-width)
	  (lookup-max-image-height ndeb-max-image-height))
      (if (and start end)
...
(defun ndeb-arrange-wave (entry)
  ...
  (let ((regexp (lookup-dictionary-option
		 (lookup-entry-dictionary entry) :wave-regexp t)))
    (while (re-search-forward (car regexp) nil t)
...
(defun ndeb-arrange-mpeg (entry)
  ...
  (let* ((regexp (lookup-dictionary-option
		  (lookup-entry-dictionary entry) :mpeg-regexp t)))
    (while (re-search-forward (car regexp) nil t)
...
(defun ndeb-arrange-snd-autoplay (entry)
  ...
  (when (and (lookup-dictionary-option
	      (lookup-entry-dictionary entry) :snd-autoplay nil)
