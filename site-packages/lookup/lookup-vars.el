;;; lookup-vars.el --- Lookup variable list -*- coding: utf-8 -*-
...
(defconst lookup-version "1.4+media-20220424"
  "Lookup のバージョン番号。")
...
(defcustom lookup-data-directory (expand-file-name "lookup"
						   data-directory)
...
(defcustom lookup-query-expansion-methods
  '(exact prefix suffix substring keyword)
  "*query expansionを行う検索方法のリスト。"
  :type '(repeat (choice (const exact)
			 (const prefix)
			 (const suffix)
			 (const substring)
			 (const wild)
...
(defcustom lookup-use-kakasi (and (boundp 'lookup-kanji-scheme)
				  lookup-kanji-scheme t)
...
(defcustom lookup-max-image-size 8388608
  "When image size in bytes is larger than it's value, image is never displayed.
Nil means unlimited."
  :type '(choice integer (const nil))
  :group 'lookup-general-options)

(defcustom lookup-max-image-width nil
...
(defcustom lookup-max-image-height nil
...
(defcustom lookup-expanded-heading-prefix "[%s ->] "
  "Format string used for prefix of entry heading searched with expanders.
